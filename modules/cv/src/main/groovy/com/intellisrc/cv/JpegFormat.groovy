...
class JpegFormat {
	 ...
	 */
	static final byte[] SOI_MARKER = [ (byte) 0xFF, (byte) 0xD8 ]
	 ...
	 */
	static final byte[] APP0_MARKER = [ (byte) 0xFF, (byte) 0xE0 ]
	 ...
	 */
	static final byte[] COM_MARKER = [ (byte) 0xFF, (byte) 0xFE ]

	/**
	 * End Of Image, size: 2 bytes
	 * The last two bytes of every JPEG image.
	 */
	static final byte[] EOF_MARKER = [ (byte) 0xFF, (byte) 0xD9 ]
	 ...
	 * Size: 69
	 */
	static final byte[] DQT_MARKER = [ (byte) 0xFF, (byte) 0xDB ]

	/**
	 * Start Of Frame (19 color/13 black&white)
	 * Size: 19/13
	 */
	static final byte[] SOF_MARKER = [ (byte) 0xFF, (byte) 0xC0 ]
	 ...
	 */
	static final byte[] DHT_MARKER = [ (byte) 0xFF, (byte) 0xC4 ]

	/**
	 * Start Of Scan (14 color/13 black&white)
	 * Size: 14/10
	 */
	static final byte[] SOS_MARKER = [ (byte) 0xFF, (byte) 0xDA ]

	/**
	 * Typical max length of the jpeg data.
	 */
	static int JPEG_MAX_LENGTH = 3 * 240 * 352
	 ...
	 * @param sequence
	 */
	static int getEndOfSeqeunce(DataInputStream ins, byte[] sequence)
		throws IOException {
		int seqIndex = 0 //tracks number of sequence chars found
		byte c

		for (int i = 0; i < (MjpegFormat.FRAME_MAX_LENGTH); i++) {
			c = (byte) ins.readUnsignedByte() //read next byte
	 ...
	 */
	static int getStartOfSequence(DataInputStream ins, byte[] sequence)
		throws IOException {
		int end = getEndOfSeqeunce(ins, sequence)

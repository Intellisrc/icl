...
class MjpegFrame {
    ...
    MjpegFrame(byte[] frame, int jpegLen, int sequence) {
        mData = frame
        mJpegLen = jpegLen
        mSeq = sequence
    }
     ...
     */
    MjpegFrame(byte[] jpegBytes, int sequence = 1) {
        mJpegLen = jpegBytes.length
        mSeq = sequence
        
        //create the MJPEG frame data
        ByteArrayOutputStream out = new ByteArrayOutputStream(jpegBytes.length + 200)

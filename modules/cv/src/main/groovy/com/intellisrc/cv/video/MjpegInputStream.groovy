 ...
 */
class MjpegInputStream extends DataInputStream {
     ...
     */
    MjpegInputStream(InputStream ins) {
        super(new BufferedInputStream(ins, MjpegFormat.FRAME_MAX_LENGTH))
    }
     ...
     */
    MjpegFrame readMjpegFrame() throws IOException {
        ...
        if (isFirstPass) {
            ...
            try {
                ...
                Log.w("couldn't parse content length from header on first pass")
        ...
        if (isContentLengthAvailable) {
            ...
            try {
                ...
                Log.w("couldn't parse content length, failover to jpeg EOF search")

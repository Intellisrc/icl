...
class Oracle extends JDBCServer implements AutoJDBC {
    ...
    @Override
    String getFieldForQuery(String field) {
        // Oracle expects uppercase identifiers when quoted
        return fieldsQuotation + field.toUpperCase() + fieldsQuotation
    }
    /**
    ...
    String getLastIdQuery(String table, String pk) {
        ...
        String uPk = (pk ?: "id").replace(fieldsQuotation, "").toUpperCase()
        return "SELECT COALESCE(MAX(\"${uPk}\"), 0) as lastId FROM \"${uTable}\""
    }
    /**
    ...
    String getCreateTableSQL(String tableName, TableDefinition definitions) {
        ...
        definitions.each { ColumnDefinition col ->
            ...
            if (col.autoIncrement) {
                parts << "GENERATED BY DEFAULT AS IDENTITY (NOCACHE)"
            }
    ...
    String getIdentitySQL(String table, String columnName) {
        String uTable = table.replace(fieldsQuotation, "").toUpperCase()
        return "SELECT (s.last_number - 1) FROM user_sequences s JOIN user_tab_identity_cols i ON s.sequence_name = i.sequence_name WHERE i.table_name = '${uTable}'"
    }

    @Override
    String getIdentityUpdateSQL(String table, String columnName, int value) {
        String uTable = table.replace(fieldsQuotation, "").toUpperCase()
        String uCol = columnName.replace(fieldsQuotation, "").toUpperCase()
        return "ALTER TABLE \"${uTable}\" MODIFY \"${uCol}\" GENERATED BY DEFAULT AS IDENTITY (RESTART START WITH ${value + 1})"
    }
